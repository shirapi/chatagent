...
export class AgentCoreStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreStackProps) {
    ...
    // --- BEGIN CUSTOM (manually added, not generated by `agentcore add`) ---
    // Grant runtime execution roles permission to read the character/model/API key
    // SSM parameters (fetched once at container startup in main.py / model/load.py).
    for (const env of this.application.environments.values()) {
      env.runtime.role.addToPrincipalPolicy(
        new iam.PolicyStatement({
          actions: ['ssm:GetParameter'],
          resources: [
            `arn:aws:ssm:${this.region}:${this.account}:parameter/chatagent/AgentCharacter`,
            `arn:aws:ssm:${this.region}:${this.account}:parameter/chatagent/AgentModelId`,
            `arn:aws:ssm:${this.region}:${this.account}:parameter/chatagent/TavilyApiKey`,
          ],
        })
      );
    }
