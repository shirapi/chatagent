...
- **コマンド実行やファイル変更など、何らかの操作をしようと思ったら、必ず先にユーザーに言う。** `go mod tidy`のような付随的な操作も含め、それをユーザー自身がやるか自分がやるかはユーザーが決める。「次はこれをやってよいか」の確認と、「これから何をするか」を告げることは別物であり、後者を省略してはいけない。
